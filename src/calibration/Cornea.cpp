...
void Cornea::create(const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> > &led_pos,		// LED locations
  ...
  for(size_t i = 0; i < sz; ++i) {
    ...
    DATA_FOR_CORNEA_COMPUTATION &cur_data = _data[i];
...
/** 
 * Calculate Cornea center
 * 
 * @param led_pos position of the led that produces each glint 
 * @param glint_pos position of the glint produced by each led (ordered 1st led->1st glint....n led->n glint)
 * @param gx_guesses initial guesses for computing
 * @param centre output with the center of the cornea
 * @param err error of the center computed
 * 
...
int Cornea::computeCentre(const std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> > &led_pos, // LED locations
  ...
  const size_t n = 3 * pairsOfTwo(_data.size());	// number of functions
  ...
  //use Nonlinear Least-Squares Fitting
  ...
  for(size_t i = 0; i < _data.size(); ++i) {
    ...
    const DATA_FOR_CORNEA_COMPUTATION &cur_data = _data[i];
  ...
  const double nof_samples = (double)_data.size();
...
void Cornea::createF(const gsl_vector *x, gsl_vector *F) const {
  ...
  for(int i = 0; i < nLEDs-1; ++i) {
    ...
    const DATA_FOR_CORNEA_COMPUTATION &data1 = _data[i];
    ...
    for(int j = i + 1; j < nLEDs; ++j) {
      ...
      const DATA_FOR_CORNEA_COMPUTATION &data2 = _data[j];
...
void Cornea::createJacobian(const gsl_vector *gx,
  ...
  for(size_t x = 0; x < nLEDs - 1; ++x) {
    ...
    const DATA_FOR_CORNEA_COMPUTATION &data1 = _data[x];
    ...
    for(size_t y = x + 1; y < nLEDs; ++y) {
      ...
      const DATA_FOR_CORNEA_COMPUTATION &data2 = _data[y];
...
/** 
 * this function stores the vector result f(x,params) in f 
 * for argument x and arbitrary parameters params, returning an 
 * appropriate error code if the function cannot be computed. 
 * 
 * @param x 
 * @param param parameters ( Cornea object )
 * @param F 
 * 
 * @return GSL_SUCCESS if success. Otherwise an error code
 */
