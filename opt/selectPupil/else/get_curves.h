	...
	for(int i=0;i<curve.size();i++){
		...
		for(float p=0.95;p>0.80;p-=0.01){//0.75;-0.05
			...
			if(p_x>0 && p_x<pic->cols && p_y>0 && p_y<pic->rows){
				...
				//debug.data[(pic->cols*p_y)+p_x]=255;
				}
...
if(add_curve){ // pupil area
	if(ellipse.size.width*ellipse.size.height < pic->cols*pic->rows*0.005 ||
		ellipse.size.width*ellipse.size.height > pic->cols*pic->rows*0.1)
		add_curve=false;
	}
...
//std::cout<<"in"<<mean_p.x<<":"<<mean_p.y<<std::endl;
					if(inner_color_range>=0){
						...
						//need check ist neu if ellipse ist in ellipse
		...
		//is there a curve in the selected curve?
